...
/*   Updated: 2015/08/08 16:21:42 by nidzik           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		ft_draw_map(t_wlf *wlf)
{
	...
	t_ray	r;
	int y = 0;
	int color_sky = 0x3366cc;
	int color_floor = 0x336600;
/* 	ft_bzero(wlf->data, ) */
...
//			pos.color = color(wlf, pos, r);
			wlf = create_text2(wlf, &r);
		y = 0;
		while (y < WIN_Y)
		{
/* 			printf("-> %d   /   %d  \n",pos.i, y);fflush(stdout); */
			if (y < pos.dstart)
			{
				wlf->data[y * wlf->sizeline + 4 * pos.i + 0] = color_sky;
				wlf->data[y * wlf->sizeline + 4 * pos.i + 1] = color_sky >> 8;
				wlf->data[y * wlf->sizeline + 4 * pos.i + 2] = color_sky >> 16;
//				printf("1");fflush(stdout);
				}	
			if (y >= pos.dstart && y <= pos.dend && wlf->texX < 64/2)
			{

			  int d = y * 256 - (pos.hwall * WIN_Y) * 128 + pos.lineh * 128;  //256 and 128 factors to avoid floats
			  int texY = ((d * texHeight) / pos.lineh) / 256;

			  wlf->data[y * wlf->sizeline + 4 * pos.i + 0] = wlf->texture[texHeight * texY + wlf->texX];
			  wlf->data[y * wlf->sizeline + 4 * pos.i + 1] = wlf->texture[texHeight * texY + wlf->texX] >> 8;
			  wlf->data[y * wlf->sizeline + 4 * pos.i + 2] = wlf->texture[texHeight * texY + wlf->texX] >> 16;
...
//				printf("2");fflush(stdout);
			...
			{
				wlf->data[y * wlf->sizeline + 4 * pos.i + 0] = color_floor;
				wlf->data[y * wlf->sizeline + 4 * pos.i + 1] = color_floor >> 8;
				wlf->data[y * wlf->sizeline + 4 * pos.i + 2] = color_floor >> 16;
...
pos.i++;
/*	printf("pos.i = %d %d %d ", pos.i, pos.dstart, pos.dend);
	while (pos.i < WIN_X)
	{
	r = ft_init_ray(r, pos);
	r = ft_side_dist(&r);
